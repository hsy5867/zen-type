# zen-type：F5 Toggle 模式（取代 Push-to-Talk）
...
| 嘗試 | 結果 |
|---|---|
| `Alt+Space` | 會打到記事本的視窗選單、修飾鍵殘留問題 |
| `Right Alt` | 作用不良（使用者回報） |
| `Right Ctrl` | 讓整個鍵盤輸入癱瘓，連手動打字都失效，必須重啟 app |
| `Left Ctrl + Left Shift + *` | 中文 IME 切換衝突、閃爍、輸入異常 |

核心問題：**只要我們在常用的修飾鍵（Ctrl / Alt / Shift）上掛 low-level hook，就會干擾 Windows 鍵盤輸入處理**，影響 IME 甚至一般打字。

使用者新要求：
1. 改用 **`F5` 單鍵**（不是修飾鍵，Windows 不會特殊處理，安全）
2. 從 **Push-to-Talk 改為 Toggle 模式**：
   - 按一次 F5 → 開始錄音並**持續錄音**（不用一直按住）
   - 再按一次 F5 → 停止錄音並處理
3. 這種模式對手部比較輕鬆，也避免「按住時其他鍵卡住」的問題
...
`config/settings.py` 的 `DEFAULT_CONFIG` 新增：
```python
"hotkeyMode": "toggle",   # "toggle" | "push_to_talk"
```

預設 `toggle`（新需求），舊的 PTT 行為仍保留為可選。

### 2. Pipeline 新增 `toggle()` 方法

`core/pipeline.py` `AudioPipeline` 加：
```python
def toggle(self, mode: Mode) -> None:
    """Toggle recording on/off. Safe to call repeatedly — first call starts,
    next call stops and processes."""
    if self._state == PipelineState.IDLE and not self._busy.is_set():
        self.start_recording(mode)
    elif self._state == PipelineState.RECORDING:
        self.stop_and_process()
    # 若在 TRANSCRIBING / POLISHING / INJECTING 中，忽略（還在處理上一次）
```

### 3. App.py 依 mode 綁 press/release

`_bind_hotkeys()` 依 `hotkeyMode`：
- `toggle`：press → `pipeline.toggle(mode)`；release → no-op
- `push_to_talk`：press → `pipeline.start_recording(mode)`；release → `pipeline.stop_and_process()`

不修改 HotkeyManager API（它照常接收 press/release 兩個 callback），在 app 層把 release 傳 no-op。

### 4. Settings UI 新增模式選擇

`ui/settings.html` 的「🎯 模式與快捷鍵」分頁最上方加：
```
[ ] 切換模式（Toggle）— 按一次開始，再按一次結束  ← 預設打勾
[ ] 按住模式（Push-to-Talk）— 按住講話，放開結束
```

### 5. 更新使用者目前 `config.json`

```json
{
  "hotkeys": { "dictate": "f5", "transform": "", "ask": "" },
  "modesEnabled": { "dictate": true, "transform": false, "ask": false },
  "hotkeyMode": "toggle"
}
```
...
- `D:\000\HSY_Project\zen-type\src\zen_type\config\settings.py` ← 新增 `hotkeyMode` 預設
- `D:\000\HSY_Project\zen-type\src\zen_type\core\pipeline.py` ← 新增 `toggle()` 方法
- `D:\000\HSY_Project\zen-type\src\zen_type\app.py` ← 依 mode 綁 callback
- `D:\000\HSY_Project\zen-type\src\zen_type\ui\settings.html` ← UI 加 toggle/PTT 選擇
- `C:\Users\SYH-w10\AppData\Roaming\zen-type\config.json` ← 使用者設定

---

## 重用既有元件

- `zen_type.core.pipeline.AudioPipeline.start_recording()` / `stop_and_process()` — toggle 直接呼叫
- `zen_type.core.hotkey.HotkeyManager` — API 不變
- `zen_type.core.injector.TextInjector`（含修飾鍵釋放）— 不變

---

## 新問題：Toggle 模式下不需要「按鍵放開」的自動停止

在 PTT 下，如果 STT 處理很久、使用者早就放開鍵，release 呼叫 `stop_and_process` 也無害（busy 檢查會 no-op）。

在 Toggle 下，按第二次 F5 時 `toggle()` 會檢查狀態：
- 若 RECORDING → 呼叫 stop_and_process → 正常流程
- 若 TRANSCRIBING/POLISHING/INJECTING（上一次還沒處理完）→ 忽略，log warning
...
1. 停掉舊 app 行程
2. 改寫 user config（hotkey=f5, hotkeyMode=toggle）
3. `uv run zen-type --debug --no-browser`
4. Log 應出現 `hotkey registered: dictate → f5`
5. 打開記事本：
   - 按一次 **F5** → overlay 變紅「● 錄音中」，開始收音；可放開手
   - 講話 5 秒
   - 再按一次 **F5** → overlay 變琥珀「辨識中…」→ 文字貼上
6. 驗證中文打字與 IME 切換不受影響（F5 不是修飾鍵，完全無干擾）
7. 測試 toggle 在處理中再按 F5 的行為（應被忽略）
